...
import { searchAirportsAPI, resolveCityToIATA } from '@/lib/api'
import type { AirportResult } from '@/lib/api'

// Comprehensive local airport data (fallback when API is down)
const ALL_AIRPORTS: AirportResult[] = [
  ...
  { iata: 'GOI', city: 'Goa', name: 'Goa International Airport', country: 'India', state: 'Goa' },
  ...
  { iata: 'CDG', city: 'Paris', name: 'Charles de Gaulle', country: 'France', state: 'Île-de-France' },
...
function searchLocal(q: string): AirportResult[] {
  ...
  return ALL_AIRPORTS.filter(a =>
    ...
    (a.state?.toLowerCase().includes(lower) ?? false)
  ).slice(0, 10)
}
...
function AirportField({ label, value, displayValue, onChange, placeholder }: AirportFieldProps) {
  ...
  const doSearch = useCallback(async (q: string) => {
    ...
    setResults(local)
    // Then try API (may enhance results)
    try {
      const api = await searchAirportsAPI(q)
  ...
  const selectAirport = (ap: AirportResult) => {
    ...
    <div ref={wrap} style={{ position: 'relative' }}>
      ...
      {open && (
        <div style={{
          ...
          boxShadow: '0 12px 32px rgba(19,18,16,.18)',
          maxHeight: '320px',
          display: 'flex',
          ...
          <div style={{ padding: '8px 12px', borderBottom: '1px solid #efefed', display: 'flex', alignItems: 'center', gap: '8px', flexShrink: 0 }}>
            ...
            <input
              ...
              placeholder="Type city name or IATA code..."
          ...
          {!query && (
            <div style={{ padding: '6px 12px 3px', fontSize: '.6rem', fontWeight: 600, letterSpacing: '.12em', textTransform: 'uppercase', color: '#c8c6c2', fontFamily: "'Martian Mono',monospace", flexShrink: 0 }}>
              Popular airports
            </div>
          )}
          ...
          <div style={{ overflowY: 'auto', flex: 1 }}>
            {loading && (
              <div style={{ padding: '12px', fontSize: '.82rem', color: '#9b9890', textAlign: 'center' }}>Searching...</div>
            )}
            {!loading && results.length === 0 && query && (
              <div style={{ padding: '12px', fontSize: '.82rem', color: '#9b9890', textAlign: 'center' }}>
            ...
            {results.map(ap => (
              <button
                ...
                style={{
                  ...
                  background: ap.iata === value ? '#f6f4f0' : 'transparent',
                  border: 'none', borderBottom: '1px solid #f0efed',
              ...
              >
                <div style={{
                  ...
                  background: ap.iata === value ? '#131210' : '#f6f4f0',
                  border: '1px solid #efefed',
                  display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0,
                }}>
                  <span style={{ fontFamily: "'Martian Mono',monospace", fontSize: '.65rem', fontWeight: 700, color: ap.iata === value ? '#fff' : '#e8191a', letterSpacing: '.04em' }}>
                    {ap.iata}
                  </span>
                </div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: '.7rem', color: '#9b9890', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                    {ap.name}
                  </div>
                </div>
                <div style={{ fontSize: '.62rem', fontWeight: 700, color: ap.country !== 'India' ? '#e8191a' : '#9b9890', fontFamily: "'Martian Mono',monospace", flexShrink: 0 }}>
                  {ap.country !== 'India' ? ap.country.toUpperCase().slice(0, 3) : ap.state?.slice(0, 3).toUpperCase()}
                </div>
              </button>
            ))}
          </div>
...
export default function FlightSearchForm({ compact = false }: { compact?: boolean }) {
  ...
  const swap = () => {
    ...
    setForm(f => ({
      ...f,
      originIata: f.destinationIata,
      originDisplay: f.destinationDisplay,
      destinationIata: f.originIata,
      destinationDisplay: f.originDisplay,
    }))
  }
