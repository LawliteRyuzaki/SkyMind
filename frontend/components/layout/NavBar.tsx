...
export default function NavBar() {
  ...
  const userInitial = user?.email?.[0]?.toUpperCase() || user?.user_metadata?.full_name?.[0]?.toUpperCase() || '?'
    ...
    <>
              ...
              user ? (
                <div style={{ display: 'flex', alignItems: 'center', gap: '0' }}>
                  <Link href="/dashboard" style={{
                    padding: '0 16px', height: '100%', display: 'flex', alignItems: 'center', gap: '8px',
                    textDecoration: 'none', fontSize: '.78rem', fontWeight: 600, color: 'var(--black)',
                    borderRight: '1px solid #efefed', transition: 'background .15s',
                  }}
                  ...
                  <button onClick={handleSignOut} style={{
                    padding: '0 16px', height: '100%', background: 'none', border: 'none', cursor: 'pointer',
                    fontSize: '.78rem', fontWeight: 500, color: '#5c5a56', borderRight: '1px solid #efefed',
                    fontFamily: "'Instrument Sans',sans-serif", transition: 'color .15s',
                  }}
      ...
      {mobileOpen && (
        <div style={{
          ...
          {user ? (
            <>
              <Link href="/dashboard" style={{ display: 'block', padding: '16px 20px', fontSize: '.9rem', color: '#5c5a56', textDecoration: 'none', borderBottom: '1px solid #efefed' }}>
                My Dashboard
              </Link>
              <button onClick={handleSignOut} style={{ display: 'block', width: '100%', textAlign: 'left', padding: '16px 20px', fontSize: '.9rem', color: '#e8191a', background: 'none', border: 'none', borderBottom: '1px solid #efefed', cursor: 'pointer', fontFamily: "'Instrument Sans',sans-serif" }}>
                Sign Out
              </button>
            </>
          ) : (
            <Link href="/auth" style={{ display: 'block', padding: '16px 20px', fontSize: '.9rem', color: '#5c5a56', textDecoration: 'none', borderBottom: '1px solid #efefed' }}>
              Sign In
            </Link>
          )}
          <Link href="/flights" style={{
