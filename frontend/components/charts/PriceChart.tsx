...
// Renders forecast array from /predict API — no hardcoded values.

import React, { useMemo } from "react";
import type { ForecastPoint, Trend } from "@/lib/api";

const TREND_COLOURS: Record<Trend, { line: string; fill: string; dash: string }> = {
  RISING:  { line: "#e8191a", fill: "rgba(232,25,26,0.10)",  dash: "rgba(232,25,26,0.35)" },
  FALLING: { line: "#166534", fill: "rgba(22,101,52,0.10)",  dash: "rgba(22,101,52,0.35)" },
  STABLE:  { line: "#2563eb", fill: "rgba(37,99,235,0.10)",  dash: "rgba(37,99,235,0.35)" },
};
  ...
  return (
    <div style={{ width: "100%", overflowX: "auto" }}>
      <svg
        viewBox={`0 0 ${W} ${H}`}
        width="100%"
        style={{ display: "block", fontFamily: "var(--fm)" }}
      >
        {/* Grid lines */}
        ...
        <g transform={`translate(${PAD.left + 4},${H - 10})`}>
          ...
          <line x1={100} x2={114} y1={0} y2={0} stroke={colours.dash} strokeWidth={1} strokeDasharray="4 4" />
          <text x={118} y={4} fontSize={9} fill="var(--grey3)">Confidence Interval</text>
        </g>
      </svg>
    </div>
  );
